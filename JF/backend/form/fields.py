...
class Field:
    ...
    def match(self, name, value):
        ...
        2、否则。检测value是否输入了。没有输入。去检查子类中是否约定由错误提示信息。如果没有给定一个默认错误提示
                如果value输入了。执行正则匹配match。
        ...
        if not self.required:  # 判断可以为空，self.required来自各小类中
            ...
            if not value:#没有获取到值
                if self.custom_error_dict.get('required',None):#去子类中找对应的错误信息提示
                ...
                if ret: #验证正确
                    ...
                    if self.custom_error_dict.get('valid',None):#去子类中找对应错误信息提示
