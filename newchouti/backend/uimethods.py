...
<li class="items" style='padding:8px 0 0 %spx;'> 
    <span class="folder" id='comment_folder_%s'> #评论id
        <div class="comment-L comment-L-top"> #评论的头像
        ...
        <div class="comment-R comment-R-top" style="background-color: rgb(246, 246, 246);">
            <div class="pp">
                <a class="name" href="/user/moyujian/submitted/1">%s</a>#用户名
                <span class="p3">%s</span> #评论的内容
                <span class="into-time into-time-top">%s</span>#评论时间
            </div>
            <div class="comment-line-top">
                <div class="comment-state">
                    ...
                    <span class="line-huifu">|</span>
                    <a class="see-a huifu-a" href="javascript:void(0);" onclick="reply(%s,%s,'%s')"  id='comment_reply_%s' >回复</a>#回复
                </div>
            </div>
        </div>
    </span>
...
def generate_comment_html(sub_comment_dic, margin_left_val):
    ...
    for k, v_dic in sub_comment_dic.items():
        ...
        if v_dic:
            html += generate_comment_html(v_dic, margin_left_val)#递归
        html += "</li>"
    html += "</ul>"
    return html
...
def tree(self, comment_dic):
    ...
    for k, v in comment_dic.items():
        #k (2, '222222', None, 'cjx', datetime.datetime(2017, 11, 16, 15, 49, 48), 0, 0, 1)
        #0，margin的距离，
